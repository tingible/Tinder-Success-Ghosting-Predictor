...
with st.container():
    ...
    with col1:
        Num_of_likes_vs_swipes = st.number_input('% of right swipes', min_value=1, max_value=100)
    with col2:
...
with st.container():
    ...
    with col5:
        Positive_Neutral_percent = st.slider('% of positive or okay vibes', 0, 100, 50)

# Features calculations
Pickiness_indicator = Num_of_likes_vs_swipes / 100
Attractiveness_indicator = Num_of_matches_vs_likes / 100
Positive_Neutral_proportion = Positive_Neutral_percent / 100
...
y_pred = model.predict([[Num_messages, Message_word_count, Pickiness_indicator, Positive_Neutral_proportion, Attractiveness_indicator]])
...
input_feature = pd.DataFrame([[Pickiness_indicator, Attractiveness_indicator, Num_messages, Message_word_count, Positive_Neutral_proportion]],
                            columns=['Pickiness Indicator','Attractiveness Indicator','Number of messages exchanged',
                                     'Estimated length of messages in word count', '% of positive/okay vibes'])
